...
export function reconcileChildren(
  ...
  if (current === null) {
    //如果这是一个尚未渲染的全新组件，我们不会通过应用最小的副作用来更新其子集。相反，
    //在渲染之前，我们将它们全部添加到子孙节点中。
    //那意味着我们可以通过不跟踪副作用来优化这个过程。
    ...
    // 如果当前子项与正在进行的任务相同，这意味着我们尚未对这些子项开始任何工作。
    // 因此，我们使用克隆算法来创建所有当前子项的副本。
    // 如果我们之前有任何已进展的工作，此时它是无效的，所以让我们将其丢弃。
...
function updateFunctionComponent(
  ...
  if (current !== null && !didReceiveUpdate) {
    bailoutHooks(current, workInProgress, renderLanes);
    console.log("bailout in BeginWork");
    return bailoutOnAlreadyFinishedWork(current, workInProgress, renderLanes);
  }
