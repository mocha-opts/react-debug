...
function mountWorkInProgressHook() {
  ...
  //（wipHook指向链表的末尾）如果wipHook指针不存在，那么这就是第一个hook，后面继续将新的hook添加到末尾
...
//首次渲染 Rerender 阶段 和 后续的 Update 阶段共用
function updateWorkInProgressHook() {
  ...
  let nextCurrentHook;
  // 如果这是组件里第一个调用的 Hook
  // 如果是首次渲染的rerender阶段，currentHook就是null
  if (currentHook === null) {
    //如果是首次渲染的rerender阶段 current 也是null,如果是update的话就从旧current中拿第一个hook节点的指针
    const current = currentlyRenderingFiber.alternate;
    if (current !== null) {
      nextCurrentHook = current.memoizedState;
    } else {
      // 第一次渲染 This is the initial render.
  ...
  //这就是处理 renderWithHooksAgain 的关键。
  // 在重跑时，WIP Fiber 上已经有第一次运行创建好的 Hook 了，我们应该直接复用它们，而不是重新创建。
  let nextWorkInProgressHook;
  if (workInProgressHook === null) {
    // 尝试获取 WIP Fiber 上已经存在的第一个 Hook
  ...
  if (nextWorkInProgressHook !== null) {
    //命中此分支说明：这是 Render Phase Update 导致的重跑（Re-render）
    ...
    // 命中此分支说明：这是正常的 Update，或者是重跑时发现 Hook 数量不对
    if (nextCurrentHook === null) {
      const currentFiber = currentlyRenderingFiber.alternate;
      // 错误处理：如果 current 里没 Hook 了，说明用户这次渲染比上次多写了 Hook
    ...
    currentHook = nextCurrentHook; // 同步当前参照指针
    // 创建一个新的 Hook 对象，克隆旧状态
    const newHook = {
    ...
    if (workInProgressHook === null) {
      // This is the first hook in the list.
      // 如果是链表头，挂载到 Fiber.memoizedState
...
function mountReducer(reducer, initialArg, init) {
  const hook = mountWorkInProgressHook(); //最新的hook
  let initialState;
  if (init !== undefined) {
...
function updateReducerImpl(hook, current, reducer) {
  ...
  if (baseQueue === null) {
    ...
    if (!is(newState, hook.memoizedState)) {
      console.log(
        "receiveUpdate in updateReducer and markWorkInProgressReceivedUpdate",
        newState,
        "hook.memoizedState",
        hook.memoizedState,
      );
...
function mountStateImpl(initialState) {
  // 1. 创建一个新的 Hook 对象并连接到链表末尾
  const hook = mountWorkInProgressHook();
  // 2. 初始化 Hook 的状态
  if (typeof initialState === "function") {
    const initialStateInitializer = initialState;
    // $FlowFixMe[incompatible-use]: Flow doesn't like mixed types
    initialState = initialStateInitializer(); // 如果是函数，就运行一下，初始化状态
    if (shouldDoubleInvokeUserFnsInHooksDEV) {
      setIsStrictModeForDevtools(true);
      try {
        // $FlowFixMe[incompatible-use]: Flow doesn't like mixed types
  ...
  //将initialState保存两份 一份作为基础State，也就是baseState，一份作为最新的State，也就是memoizedState
  hook.memoizedState = hook.baseState = initialState;
  // 3. 创建更新队列（用于后续 setState）
  const queue = {
    pending: null,
    lanes: NoLanes,
    dispatch: null,
...
function mountState(initialState) {
  ...
  //   lastRenderedState: initialState,
  // };
  // 4. 绑定 dispatch 函数（即你拿到的那个 setCount）
  const dispatch = dispatchSetState.bind(null, currentlyRenderingFiber, queue); // 关键：闭包锁定了当前的 Fiber 节点和更新
...
function mountSwipeTransition(previous, current, next) {
  const queue = {
    pending: null,
    dispatch: null,
...
if (enableSwipeTransition) {
  HooksDispatcherOnMount.useSwipeTransition = mountSwipeTransition;
}
...
if (__DEV__) {
  ...
  HooksDispatcherOnMountInDEV = {
    ...
    useEffect(create, createDeps, update, updateDeps, destroy) {
